import { load, loadString, save, saveString, clear, remove, storage, reduxStorage } from "./storage"
...
describe("MMKV Storage", () => {
  ...
  it("reduxStorage save strings", () => {
    reduxStorage.setItem("string", "new string").then(() => {
      reduxStorage.getItem("string").then((value: string) => {
        expect(value).toEqual("new string")
      })
    })
  })

  it("reduxStorage remove strings", () => {
    reduxStorage.removeItem("string").then(() => {
      reduxStorage.getItem("string").then((value: string) => {
